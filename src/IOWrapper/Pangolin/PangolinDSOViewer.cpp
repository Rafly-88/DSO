...
void PangolinDSOViewer::run()
{
	...
	// The GPU AnypointShader / d_anypoint panel were removed: the rectified
	// view is now produced on the CPU and is exactly what DSO sees, so it
	// already shows up in d_video. The sliders below still drive that CPU
	// remap via AnypointParams.
	...
	// ── Anypoint / panorama shader controls ─────────────────────────────────
	// Toggle the panel + pick the projection mode + 3 generic control knobs.
	// The meaning of param1/2/3 depends on the chosen mode (see header docs).
	pangolin::Var<bool>   ap_enable    ("ui.anyEnable",     true,  true);
	pangolin::Var<int>    ap_mode      ("ui.anyMode",       0,     0, 3, false);   // 0=mode1, 1=mode2, 2=tube, 3=car
	pangolin::Var<double> ap_param1    ("ui.anyP1",         0.0,  -180.0, 180.0, false);
	pangolin::Var<double> ap_param2    ("ui.anyP2",         0.0,  -180.0, 180.0, false);
	pangolin::Var<double> ap_param3    ("ui.anyP3",         1.0,   0.05,  3.0,   false);
	// Calibration knobs (override if your fisheye lens differs).
	pangolin::Var<double> ap_iCx       ("ui.any_iCx",       w * 0.5, 0.0, (double)w,   false);
	pangolin::Var<double> ap_iCy       ("ui.any_iCy",       h * 0.5, 0.0, (double)h,   false);
	pangolin::Var<double> ap_calibRatio("ui.any_calibR",    1.0,     0.1, 4.0,         false);
	pangolin::Var<double> ap_poly5     ("ui.any_poly5",     200.0,   1.0, 1000.0,      false);
	pangolin::Var<bool>   ap_reload    ("ui.anyReload",     false, false);
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		// Mirror the GUI controls into the shared params struct the camera
		// thread is reading. This is what makes the slider also drive the
		// CPU anypoint remap that DSO uses for detection.
		{
			AnypointParams ap;
			ap.mode             = ap_mode.Get();
			ap.param1           = (float)ap_param1.Get();
			ap.param2           = (float)ap_param2.Get();
			ap.param3           = (float)ap_param3.Get();
			ap.iCx              = (float)ap_iCx.Get();
			ap.iCy              = (float)ap_iCy.Get();
			ap.calibrationRatio = (float)ap_calibRatio.Get();
			ap.poly5            = (float)ap_poly5.Get();
			ap.enabled          = ap_enable.Get();
